...
import math #to calculate side of visualization square
import urllib
import cgi 
...
def output_head() :
	...
	print "<TITLE>" + doctitle + "</TITLE>"
        ...
        print html.make_tag('script', '', { 'src':'../js/jquery-1.8.2.min.js'})
        print html.make_tag('script', '', { 'src':'../js/20q_effects.js'})

        print html.make_tag('meta', '',  { 'http-equiv':'Content-Type', 
...
def next_question(trail, things):
        ...
        con = console.HtmlConsole()
        con.append("%d Questions" % len(questions))
        con.display
...
def transition_class(resp1, resp2):
        class_list = []
        ...
        if resp1:
                ...
                class_list.append('Unanswered')

        if resp2:
                if resp2 == 'y':
                        class_list.append('ThenYes')
                elif resp2 == 'n':
                        class_list.append('ThenNo')
        else:
                class_list.append('ThenUnanswered')
        
        if resp1 != resp2:
                class_list.append('Changed')

        return class_list
        
def elimination_class(is_eliminated):
        if is_eliminated:
                return ['Eliminated']
        return []

def print_things_list(things_before, last_q_id, last_q_resp, next_q_id):
        #import pdb ; pdb.set_trace()
        eliminated_names = [ th.name for th in things_before 
                             if conflicting_response(th, last_q_id, last_q_resp) ]

        #names = [ tq.thing_name(th) for th in things_before ]
        names = [ th.name for th in things_before ]

        transitions = [ transition_class(th.response(last_q_id), 
                                         th.response(next_q_id)) 
                        for th in things_before ]

        eliminated  = [ elimination_class(conflicting_response(th, last_q_id, last_q_resp)) 
                        for th in things_before ]

        transitions = [ ' '.join(trans + elim) for trans, elim in zip(transitions, eliminated)]
...
#        lis  = [ html.make_tag(first_name(tq.thing_name(th)), 'li', tag_classes(th, selected_q_id, eliminated_things)) for th in things ]
        lis  = [ html.make_tag('li', 
                               '', 
                               {'class':trans, 'id':th.first_name}) 
                 for th, trans in zip(things_before, transitions) ]

        lis.append('<br style="clear:both" />')

        num_all_things = len(tq.db_load_things())
        square_side = math.ceil(math.sqrt(num_all_things)) * 12

        print html.make_tag('ul', '\n'.join(lis), { 'class':'Things', 'style':'width:' + str(square_side) + 'px' } )
...
def process_eliminate(trail):
	...
	things    = tq.db_load_things()
        ...
        #for visualization, save the state before the last question 
        #so we can fade transition eliminated and switched things

        #remove the last response on the trail
        trail_tuples = tq.trail_to_tuple_list(trail)
        last_q_id, last_q_resp = trail_tuples[-1]

        responses_except_last = tq.trail_to_responses(trail)
        responses_except_last.pop(last_q_id)

        things_before = eliminate_things_against_responses(things, responses_except_last)
        things        = eliminate_things_against_responses(things_before, { last_q_id:last_q_resp })
	...
	if len(things) == 1 :
		prompt_confirm_guess(things.pop(), filter_skipped_questions(trail), True) 
                ...
                next_q = next_question(trail, things)
                next_q_id = tq.question_id(next_q)
                ...
                if len(things) > 1 :	#can't make a guess yet
                        if next_q :	#keep asking
                                ...
                                con.append("%d Things" % len(things))
                                con.append("Responses %d <span class=\"Yes\">%d</span> <span class=\"No\">%d</span>" % 
                                         tq.response_counts(next_q_id, things))
                                con.append("Evenness: %f" % dbg_evenness)
                                con.append("Trail:%s" % trail)

                                #con.display()
                                
                                print '<div class="ThingsBlockContainer">'
                                print_things_list(sorted(things_before), last_q_id, last_q_resp, next_q_id)
                                ...
                                # Pick thing with most responses, as others with fewer are likely to have
                                # responses that would have eliminated them had the question been asked.
                                sorted_things = sorted(things, key = lambda th : len(th.responses))
...
def process_confirm_guess(trail, guess_name, is_it_you):
	if is_it_you == 'y' :
		...
		if trail : #None if the first guess is correct
                        identity_responses = tq.db_get_thing_by_name(guess_name).responses
                        your_responses     = tq.trail_to_responses(trail)
                        merged_responses   = dict(identity_responses.items() + your_responses.items())

                        updated_identity = tq.Thing(guess_name, tq.responses_to_trail(merged_responses))
...
def process_identity_question(trail, your_name, guess_name):
        #TODO: first check if there is already an identity question for either your_name or guess_name
        q_text_you   = "{s!%s}" % your_name
        q_text_guess = "{s!%s}" % guess_name

        questions = tq.db_load_questions()
        
        q_identity_you   = tq.find_question_by_text(q_text_you, questions)
        q_identity_guess = tq.find_question_by_text(q_text_guess, questions)

        if q_identity_you:
                q_id = tq.question_id(q_identity_you.pop())
                db_update_distinguishing_question(your_name, guess_name, q_id, 'y')
        elif q_identity_guess:
                q_id = tq.question_id(q_identity_guess.pop())
                db_update_distinguishing_question(your_name, guess_name, q_id, 'n')
        else:
                #create a new identity question from your name
                q_id = new_q_id(tq.db_load_questions())
                db_add_question(tq.make_question(q_id, q_text_you))
...
def process_get_question(your_name, guess_name, selected_q, q_text, resp, q_filter=None):
        if selected_q:
                ...
                q_id = new_q_id(tq.db_load_questions())
                db_add_question(tq.make_questions(q_id, q_text))
...
def db_update_distinguishing_question(your_name, other_name, q_id, your_resp):
        """Update the database so that you answer your_resp, and other_name answers the opposite, to q_id"""
        you_thing = tq.db_get_thing_by_name(your_name)
        you_thing.responses[q_id] = your_resp
        tq.db_update_thing(you_thing) 
	
        other_thing     = tq.db_get_thing_by_name(other_name)
        other_thing.responses[q_id] = your_resp
        toggle_response(other_thing.responses, q_id)
...
def prompt_next_answer(question, trail) :
	if is_identity_question(question):
        ...
        print '<div class="ResponsePrompt">'

        for resp in ('Yes', 'No', 'Skip'):
                params['trail'] = old_trail + '-' + q_id + resp[0:1].lower()
                class_attr = ' class="' + resp + '"'
                print "<a href=\"?%s\"%s>%s</a>" % (urllib.urlencode(params), class_attr, resp)

        print '</div>'
        #response is processed by process_eliminate

def prompt_confirm_guess(guess_thing, trail, is_sure=False) :
        guess_name = guess_thing.name
        if is_sure:
...
def conflicting_response(thing, q_id, resp):
        ...
        return not same_response_or_skipped(thing.responses, q_id, resp)
...
#		return [ th for th in things if same_response_or_skipped(tq.thing_responses(th), q_id, q_r) ]
		return [ th for th in things if same_response_or_skipped(th.responses, q_id, q_r) ]
	return []
...
def get_distinguishing_questions(thing1, thing2) :
	...
	q1 = set(get_qids(thing1.responses)) 
	q2 = set(get_qids(thing2.responses))
...
#######################
# database file
# : most stuff is in module, but adding things/questions can be done only by playing the game, so they are here

...
def output_head(doctitle_suffix = '') :
	...
	print '<a href="20q_edit.py">Edit</a>'
	print '<hr>'
        print '</div>'


        print '<div class="PageTitle">'
	print "<H1><code>" + "log<sub>2</sub>len(fb_friends)</code> Editor" + "</H1>"
        print '</div>'
...
#question_id in mod
def trail_qids(trail) :
	return [s[:-1] for s in as_array(trail)]

def get_qids(trail) :
	return [step[:-1] for step in as_array(trail)]
...
def action_remove_responder(q_id, things_to_remove):
	...
	for name in things_to_remove:
		...
		responses = tq.thing_responses(thing)
...
def print_questions_list(questions, things=None, selected_q_id=None, selected_th_name=None):
        ...
        if selected_th_name:
                ...
                class_list = [ response_class(tq.thing_response_to_q(thing, q_id)) for q_id in q_ids ]
        else:
        ...
        anchors  = [ html.make_tag(q, 'a', tag_args) for (q, tag_args) in zip(questions, tag_args_list) ]
        ...
        lis = [ html.make_tag(anc, 'li') for anc in li_items_list ]
...
def print_things_list(things, selected_th_name=None, selected_q_id=None):
        ...
        #the anchors contain both href and CSS class for selected question, or response to selected thing
        anchors  = [ html.make_tag(first_name(name), 'a', tag_args) for (name, tag_args) in zip(names, tag_args_list) ]
...
def edit_by_question(things, questions, q_id):
        ...
        print "\t<input type=submit value='Remove Responses by the checked people to %s'>" % question
        print "<br>\t<a href=\"?action=remove_question&q_id=%s\">Comment out question: %s</a>" % (q_id, question)
        print '</div>'
        print '</form>'
